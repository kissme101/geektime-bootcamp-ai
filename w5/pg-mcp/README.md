...
3. **表/列访问控制**：可通过 `SECURITY_BLOCKED_TABLES` / `SECURITY_BLOCKED_COLUMNS` 禁止访问敏感对象（列支持 `表.列` 限定名），在 SQL 验证阶段直接拒绝
4. **EXPLAIN 策略**：默认禁用 EXPLAIN，可通过 `SECURITY_ALLOW_EXPLAIN` 显式开启
5. **只读角色**：可配置 `SECURITY_READONLY_ROLE`，每次查询 `SET ROLE` 到该角色，在数据库层面叠加权限约束
6. **SQL 解析**：使用 sqlglot 进行准确的 SQL 结构验证
7. **注入防护**：参数化查询和输入清理
8. **资源限制**：
   - 行数限制（默认：10,000）
   - 查询超时（默认：30 秒）
   - 问题长度限制（`VALIDATION_MAX_QUESTION_LENGTH`）
   - 连接池管理
9. **事务隔离**：所有查询在只读事务中运行
...
- **熔断器**：防止级联 LLM API 失败
- **限流**：LLM 调用与数据库查询各自独立限流，超时未获槽位返回 `RATE_LIMIT_EXCEEDED`
- **重试逻辑**：验证失败携带错误反馈重试；瞬时 LLM 故障采用指数退避重试
- **连接池**：高效的数据库连接复用
- **Schema 缓存**：基于 TTL 的缓存减少数据库元数据查询

### 多数据库支持

主数据库通过 `DATABASE_*` 变量配置且始终可用。若需在同一服务中服务多个数据库，将 `EXTRA_DATABASES` 设置为数据库配置对象的 JSON 数组即可；每个查询通过工具的 `database` 参数选择目标库。每个数据库都拥有**独立的连接池与执行器**，因此查询始终在正确的库上执行，敏感对象也能得到隔离保护。
...
| `SECURITY_BLOCKED_TABLES`         | 禁止访问的表(逗号分隔/JSON) | 空(不限制)     |
| `SECURITY_BLOCKED_COLUMNS`        | 禁止访问的列，支持 `表.列` 限定名 | 空(不限制) |
| `SECURITY_ALLOW_EXPLAIN`          | 是否允许 EXPLAIN 语句     | `false`           |
| `SECURITY_READONLY_ROLE`          | 每次查询 SET ROLE 的只读角色 | 无             |
| `SECURITY_SAFE_SEARCH_PATH`       | 查询会话的安全 search_path | `public`         |
...
| `RESILIENCE_RATE_LIMIT_QUERIES`        | 最大并发数据库查询数 | `10` |
| `RESILIENCE_RATE_LIMIT_LLM`            | 最大并发 LLM 调用数 | `5`  |
| `RESILIENCE_RATE_LIMIT_TIMEOUT`        | 获取限流槽位的超时（秒） | `30` |
