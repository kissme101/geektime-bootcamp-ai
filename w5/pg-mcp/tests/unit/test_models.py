...
class TestQueryResponseToDict:
    """Tests for the single, canonical QueryResponse.to_dict() implementation."""

    def test_single_to_dict_definition(self) -> None:
        """There must be exactly one to_dict on the class (no shadowed duplicate)."""
        # A duplicate ``def to_dict`` in the source would silently shadow the
        # first; guard against that regression by checking it is defined once.
        import inspect

        from pg_mcp.models import query as query_module

        source = inspect.getsource(query_module.QueryResponse)
        assert source.count("def to_dict(") == 1

    def test_tokens_used_present_when_none(self) -> None:
        """tokens_used is always present in the dict, defaulting to 0 when None."""
        response = QueryResponse(
            success=True,
    ...
    def test_tokens_used_preserved_when_set(self) -> None:
        """A real token count is preserved by to_dict."""
        response = QueryResponse(
            success=True,
            generated_sql="SELECT 1",
            confidence=100,
        ...
        assert response.to_dict()["tokens_used"] == 234

    def test_none_fields_are_excluded(self) -> None:
        """Null fields (other than tokens_used) are omitted for a compact payload."""
        response = QueryResponse(
            success=True,
            generated_sql="SELECT 1",
            confidence=100,
        ...
        result = response.to_dict()
        assert "error" not in result  # None error omitted
        assert "data" not in result  # None data omitted
        assert result["success"] is True
        assert result["tokens_used"] == 0  # but tokens_used stays

    def test_error_response_to_dict(self) -> None:
        """Error responses serialize the error and still carry tokens_used."""
        from pg_mcp.models.query import ErrorDetail
