...
class SQLGenerator:
    ...
    def __init__(self, config: OpenAIConfig) -> None:
        ...
        # Token usage reported by the most recent successful ``generate`` call.
        # ``None`` when the provider did not return usage metadata.
    ...
    async def generate(
        ...
        # Record token usage (if the provider reported it) for observability.
        self.last_tokens_used = self._extract_token_usage(response)

        return sql

    @staticmethod
    def _extract_token_usage(response: "ChatCompletion") -> int | None:
        """Extract total token usage from an OpenAI response, if available.

        Args:
            response: The chat completion response.

        Returns:
            int | None: Total tokens used, or None if usage metadata is absent
                or not an integer (e.g. when the response is a test mock).
        """
