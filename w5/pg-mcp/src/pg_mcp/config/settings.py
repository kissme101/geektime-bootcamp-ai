...
class SecurityConfig(BaseSettings):
    ...
    blocked_tables: list[str] = Field(
        ...
        description="Tables that must never be accessed (comma-separated or JSON list)",
    )
    blocked_columns: list[str] = Field(
        default_factory=list,
        description=(
            "Columns that must never be accessed. Supports bare names ('password') or "
            "qualified names ('users.password'); comma-separated or JSON list"
        ),
    )
    ...
    @classmethod
    def parse_blocked_identifiers(cls, v: str | list[str] | None) -> list[str]:
        """Parse comma-separated string or list into a list of identifiers."""
        if v is None:
            return []
        if isinstance(v, str):
...
class ValidationConfig(BaseSettings):
    ...
    min_confidence_score: int = Field(
        ...
        ge=0,
        le=100,
        description="Minimum confidence score (0-100) for results to be considered acceptable",
    )
...
class ResilienceConfig(BaseSettings):
    ...
    rate_limit_timeout: float = Field(
        ...
        ge=0.1,
        le=300.0,
        description="Seconds to wait for a rate-limit slot before rejecting the request",
    )
...
class Settings(BaseSettings):
    ...
    # Additional databases beyond the primary ``database`` above. Provide a JSON array of
    # DatabaseConfig dicts via the EXTRA_DATABASES environment variable, e.g.
    #   EXTRA_DATABASES='[{"name": "analytics", "host": "db2", "user": "ro", "password": "x"}]'
    extra_databases: str = Field(
        default="",
        description="JSON array of additional database configurations (see all_database_configs)",
    )
    ...
    def all_database_configs(self) -> dict[str, DatabaseConfig]:
        """Return every configured database keyed by database name.

        The primary ``database`` is always included. Any entries parsed from
        ``extra_databases`` (a JSON array) are added afterwards. On a name
        collision the primary database wins so it can never be shadowed.

        Returns:
            dict[str, DatabaseConfig]: Mapping of database name to its config.

        Raises:
            ValueError: If ``extra_databases`` is not a valid JSON array of objects.
        """
        configs: dict[str, DatabaseConfig] = {self.database.name: self.database}

        raw = self.extra_databases.strip()
        if not raw:
            return configs
        ...
        try:
            ...
            raise ValueError(f"extra_databases must be valid JSON: {e}") from e

        if not isinstance(entries, list):
            raise ValueError("extra_databases must be a JSON array of objects")

        for entry in entries:
            if not isinstance(entry, dict):
                raise ValueError("each extra_databases entry must be a JSON object")
            cfg = DatabaseConfig.model_validate(entry)
            # Primary database wins on name collision; earlier extras win over later ones.
            configs.setdefault(cfg.name, cfg)
