...
class QueryResponse(BaseModel):
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        Null fields are omitted for a compact payload, with one exception:
        ``tokens_used`` is always present (defaulting to ``0``) because clients
        rely on it being a non-negative integer regardless of whether the LLM
        reported usage.
