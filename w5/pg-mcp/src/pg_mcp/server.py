...
async def lifespan(_app: FastMCP) -> AsyncIterator[None]:
    ...
    try:
        ...
        # 3. Create database connection pools (primary + any extra databases)
        logger.info("Creating database connection pools...")
        _pools = {}
        db_configs = _settings.all_database_configs()
        for db_name, db_cfg in db_configs.items():
            pool = await create_pool(db_cfg)
            _pools[db_name] = pool
            logger.info(
        ...
        # SQL Validator (table/column access controls and EXPLAIN policy from config)
        ...
        # SQL Executor (one per database, each bound to its own pool and config)
        ...
        # Rate Limiter (limits configurable via resilience settings)
        _rate_limiter = MultiRateLimiter(
            query_limit=_settings.resilience.rate_limit_queries,
            llm_limit=_settings.resilience.rate_limit_llm,
        )

        # 8. Create QueryOrchestrator with all resilience and observability wired in
        logger.info("Creating query orchestrator...")
        _orchestrator = QueryOrchestrator(
            sql_generator=sql_generator,
            sql_validator=sql_validator,
            sql_executor=sql_executors.get(_settings.database.name),  # fallback executor
            result_validator=result_validator,
            schema_cache=_schema_cache,
            pools=_pools,
            resilience_config=_settings.resilience,
            validation_config=_settings.validation,
            sql_executors=sql_executors,  # per-database routing
            metrics=_metrics,  # metrics recorded throughout the pipeline
            rate_limiter=_rate_limiter,  # LLM/query rate limiting
        )
...
async def query(
    ...
    try:
        ...
        # QueryResponse.to_dict() guarantees tokens_used is always present.
